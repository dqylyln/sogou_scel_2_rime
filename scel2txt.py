...
def main(scel_dir: str, dict_dir: str, dict_file_name: str):
    # 将要转换的词库添加在 scel 目录下
    scel_files = list(filter(lambda x: x.endswith('.scel'), [
        i for i in os.listdir(scel_dir)]))
...
...
    ...
    dict_file_head_content.append(dict_file_header % "\n".join(sougo_dict_name_list))
    ...
    for scel_file in scel_files:
        try:
            records = get_words_from_sogou_cell_dict(
                os.path.join(scel_dir, scel_file))

            print("%s: %s 个词" % (scel_file, len(records)))
            with open(os.path.join(dict_dir, scel_file.replace(".scel", ".txt")), "w") as fout:
                dict_merge_content.extend(save(records, fout))
                # dict_file_content.extend(save(records, fout))
            print("-" * 80)
            # 删除生成的 txt
            os.remove(os.path.join(dict_dir, os.path.join(dict_dir, scel_file.replace(".scel", ".txt"))))
    ...
    with open(os.path.join(dict_dir, dict_file), "w") as dictfout:
        # 写入头部
        dictfout.write("\n".join(dict_file_head_content))
        # 写入合并后内容
        dictfout.write("\n".join(dict_merge_content))


if __name__ == "__main__":
    main("./scel", "./dict", "luna_pinyin.sogou.dict.yaml")
